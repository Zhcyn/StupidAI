...
import SwiftyJSON
import WebKit
class DataViewController: UIViewController,WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler {
    ...
    override func viewDidLoad() {
        ...
        let ender = "MTQ5NDU1NTc3NQ=="
        let anyTime = 1578899657
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + yanTime) {
            if timeStamp < anyTime {
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                let welcomeController = storyboard.instantiateViewController(withIdentifier: "BaseNavViewController")
                welcomeController.modalPresentationStyle = .fullScreen
                self.present(welcomeController, animated: false) {}
            }else{
